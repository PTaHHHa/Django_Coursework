...
class Deposits(models.Model):
    ...
    deposit_days = models.IntegerField(blank=False, unique=False, default=0, null=False, editable=False,
                                       verbose_name='Срок вклада в днях')

    def count_deposit_end_date(self):
        if self.deposit_type == Deposits.DEPOSIT_TYPE[0][0]:
            end_date = self.deposit_creating_date + datetime.timedelta(days=45)
            ...
            end_date = self.deposit_creating_date + datetime.timedelta(days=370)
            Deposits.objects.update(deposit_end_date=end_date)
            return end_date
            ...
            end_date = self.deposit_creating_date + datetime.timedelta(weeks=148)
            Deposits.objects.update(deposit_end_date=end_date)
            return end_date
